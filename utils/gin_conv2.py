...
class GINConv2(MessagePassing):
    ...
    def __init__(self, nn, nn2, eps1=0, eps2=0, train_eps=False, **kwargs):
        super(GINConv, self).__init__(aggr='add', **kwargs)
        self.nn = nn
        self.nn2 = nn2
        self.initial_eps1 = eps1
        self.initial_eps2 = eps2
        if train_eps:
            self.eps1 = torch.nn.Parameter(torch.Tensor([eps1]))
            self.eps2 = torch.nn.Parameter(torch.Tensor([eps2]))
        else:
            self.register_buffer('eps1', torch.Tensor([eps1]))
    ...
    def reset_parameters(self):
        reset(self.nn)
        reset(self.nn2)
        self.eps1.data.fill_(self.initial_eps1)
        self.eps2.data.fill_(self.initial_eps2)
    ...
    def forward(self, x, edge_index):
        ...
        x = x.unsqueeze(-1) if x.dim() == 1 else x
        edge_index, _ = remove_self_loops(edge_index)
        out = self.nn((1 + self.eps1) * x) + self.nn2(self.propagate(edge_index, x=x))
        return out
