...
class TrajectoryDataset(Dataset):
    ...
    def __init__(
        ...
        # list all the directories at the data_dir 
        all_files = os.listdir(self.data_dir)
        
        # create a string list with the path for all the folers at data_dir
        ...
        for path in tqdm(all_files):
            ...
            ## Creating the frame_data including all the vehicles in the frame
            for frame in frames:                
                frame_data.append(data[frame == data[:, 0], :])

            ## Total number of frames in the datafile 
            num_sequences = int(
            ...
            for idx in (range(0, num_sequences * self.skip + 1, skip)):
                ...
                for _, ped_id in enumerate(peds_in_curr_seq):
                    ...
                    if (curr_ped_seq.shape[1]>=40):  
                    # print("in curr_ped in traj.py") 
                        _idx = num_peds_considered
                        curr_seq[_idx, :, pad_front:pad_end] = curr_ped_seq
                        curr_seq_rel[_idx, :, pad_front:pad_end] = rel_curr_ped_seq
                        curr_frame_id[_idx, :, pad_front:pad_end] = curr_ped_frame_id
